...
def main(args):
    ...
    try:
        ...
        print(e)
        exit()
    results_with_files = list(zip(valid_image_file_list, rec_res))

    # 按照图像文件名排序结果
    sorted_results = sorted(results_with_files, key=lambda x: x[0])
    file_name = "predictions.txt"
    with open(file_name, 'w') as file:
        for idx, (image_file, result) in enumerate(sorted_results, start=1):
            file.write("Prediction {} for {}: {}\n".format(idx, image_file, result))
