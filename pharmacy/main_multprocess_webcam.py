...
class MainProcess:
    ...
    def referer(self):
        try:
            while True:
                prescription, res_array = self._scan_prescription(self.capture_frame())
                # print(prescription)
                if res_array[0][0] in prescription and res_array[1][0] in prescription:
                    logging.info("Detected,running process started")
                    if self.running_process is not None and self.running_process.is_alive():
                        self.running_process.terminate()
                        logging.info("Shutdown previous running process")
    ...
    def process_stream(self,prescription):
        ...
        while True:
            ...
            logging.info("Drug is match, is it true?"+str(drug_match))
