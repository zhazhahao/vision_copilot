...
def image_put_thread(exit_flag,shm,config,rtsp_client_path):
    ...
    try:
        ...
        while not exit_flag.value:
            # 读取单帧图像数据
            # a = time.time()
            raw_image = ffmpeg_process.stdout.read(config.shared_memory_size)  # 假设单帧图像大小不超过 65536 字节
...
class CameraProcessor:
    def __init__(self,doaudio=True,rtsp_client_path="rtsp://192.168.8.100/live"):
        ...
        if doaudio:
            self.processes = [
                mp.Process(target=image_put_thread,args=(self.exit_flag,self.shm,self.config,self.rtsp_client_path)),
