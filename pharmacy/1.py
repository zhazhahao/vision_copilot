...
from dependency.ocr.data import create_operators, transform
...
class TextDetector(BaseOCRV20):
    def __init__(self, args, **kwargs):
        ...
        providers = [
    ("CUDAExecutionProvider", {"cudnn_conv_algo_search": "DEFAULT", "do_copy_in_default_stream": "False"}),]

        opts = onnxruntime.SessionOptions()
        opts.enable_profiling = True
        self.ort_session = onnxruntime.InferenceSession("model.onnx",opts,providers=providers)
        self.net.eval()
        if self.use_gpu:
            self.net.cuda()
...
# 创建示例输入
input_data = torch.randn(1, 3, 64, 64)  # 假设输入图像尺寸为 64x64

# 使用 torch.jit.trace 跟踪模型
traced_model = torch.jit.trace(model, input_data)

# 导出跟踪后的模型为 ONNX 格式
torch.onnx.export(traced_model, input_data, "model.onnx", verbose=True)
